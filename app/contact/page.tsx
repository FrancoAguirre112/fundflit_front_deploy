        ...
        <div className="md:w-[50%] w-full text-white">
          ...
          <div className="md:my-28 mt-10 mb-5">
            {contactItems.direct.map((item, index) => (
              <div
                key={index}
                className="flexStart rounded-md border-2 border-transparent hover:border-green-50 hover:bg-[#0d3052] w-fit cursor pointer p-3 transition-all"
              >
          ...
          <div className="flexStart">
            {contactItems.socilaMedia.map((item, index) => (
              <div
                key={index}
                className=" hover:bg-green-50 rounded-full flexCenter w-10 h-10 cursor-pointer mr-5 transition-all "
              >
                <Link href={item.link}>
                  <item.imgUrl size={20} />
                </Link>
